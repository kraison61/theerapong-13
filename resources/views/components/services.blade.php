...
{{-- 1. Section ด้านนอกสุดปล่อยให้กว้าง w-full เพื่อให้ bg-surface เต็มจอ --}}
<section id="services" data-carousel class="w-full bg-surface py-20 lg:py-28">

    {{-- 2. Container ด้านในทำหน้าที่จำกัดความกว้างและจัดกึ่งกลาง --}}
    <div class="mx-auto max-w-7xl px-6">
        ...
        <div data-carousel-track role="region" aria-label="รายการบริการ" tabindex="0"
            class="mt-12 flex snap-x snap-mandatory gap-5 overflow-x-auto scroll-smooth pb-4 -mx-6 px-6 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 rounded-2xl">

            @foreach ($services as $svc)
                {{-- แก้ไขการเรียกใช้งาน Route ให้เป็น Object Property ทั้งหมดเพื่อความ Clean --}}
                <a href="{{ route('services.index', $svc->slug) }}"
                    class="group flex flex-col rounded-2xl border border-line bg-white p-7 transition hover:border-accent hover:-translate-y-1 hover:shadow-2xl hover:shadow-accent/15 snap-start shrink-0 w-[78%] sm:w-[45%] lg:w-[31%]">
                    ...
                    <div class="mt-5 pt-4 border-t border-dashed border-line flex gap-5 text-[13px] text-muted">
                        {{-- แก้ไข Syntax number_format และการดึงความสัมพันธ์ Relation (สมมติว่า Service HasMany Prices)
                        --}}
                        <span>
                            เริ่มต้น
                            <strong class="text-navy-900 font-semibold">
                                {{ $svc->prices->first() ? number_format($svc->prices->first()->price, 0) : '0' }}.-/
                                {{ $svc->prices->first()?->unit ?? 'หน่วย' }}
                            </strong>
                        </span>
